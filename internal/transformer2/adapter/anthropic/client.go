...
func convertCanonicalMessageToAnthropicContent(msg canonical.Message) []ContentBlock {
	...
	// Handle top-level Reasoning field only when a genuine signature is available.
	if msg.Reasoning != nil && *msg.Reasoning != "" && msg.ReasoningSignature != nil && *msg.ReasoningSignature != "" {
		// Check if ContentThinking is already in Content
		hasThinkingBlock := false
		for _, cb := range msg.Content {
			if cb.Type == canonical.ContentThinking && cb.Signature != "" {
...
func (a *ClientAdapter) FormatStreamChunk(ctx context.Context, chunk *canonical.Chunk) ([]byte, error) {
	...
	if len(chunk.Deltas) > 0 {
		...
		if delta.Delta.Reasoning != nil && *delta.Delta.Reasoning != "" {
			a.pendingThinking += *delta.Delta.Reasoning
		}

		// Handle reasoning signature
		if delta.Delta.ReasoningSignature != nil && *delta.Delta.ReasoningSignature != "" && a.pendingThinking != "" {
		...
		if delta.FinishReason != nil && !a.hasFinished {
			...
			if a.hasThinkingContentStarted || a.hasTextContentStarted || a.hasToolContentStarted {
				events = append(events, a.emitContentBlockStop())
			}
