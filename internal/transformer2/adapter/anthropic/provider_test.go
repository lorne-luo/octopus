...
func TestBuildRequest_ThinkingConfig(t *testing.T) {
	...
	t.Run("omits_unsigned_assistant_thinking_block", func(t *testing.T) {
		req := &canonical.Request{
			Kind:  canonical.KindChat,
			Model: "claude-sonnet-4-20250514",
			Messages: []canonical.Message{
				{Role: canonical.RoleUser, Content: []canonical.ContentBlock{{Type: canonical.ContentText, Text: "Hello"}}},
				{
					...
					Content: []canonical.ContentBlock{
						{Type: canonical.ContentThinking, Thinking: "synthetic thinking"},
						{Type: canonical.ContentText, Text: "Hi"},
					},
				},
				{Role: canonical.RoleUser, Content: []canonical.ContentBlock{{Type: canonical.ContentText, Text: "Follow up"}}},
			},
			MaxTokens: int64Ptr(1024),
		}
		...
		assistant := areq.Messages[1]
		if len(assistant.Content.Blocks) != 1 {
			t.Fatalf("Expected unsigned thinking to be omitted, got %d blocks", len(assistant.Content.Blocks))
		}
		if assistant.Content.Blocks[0].Type != ContentTypeText {
			t.Fatalf("Expected only text block, got '%s'", assistant.Content.Blocks[0].Type)
		}
	})

	t.Run("omits_message_with_only_unsigned_thinking", func(t *testing.T) {
		req := &canonical.Request{
			Kind:  canonical.KindChat,
			Model: "claude-sonnet-4-20250514",
			Messages: []canonical.Message{
				{Role: canonical.RoleUser, Content: []canonical.ContentBlock{{Type: canonical.ContentText, Text: "Hello"}}},
				{Role: canonical.RoleAssistant, Content: []canonical.ContentBlock{{Type: canonical.ContentThinking, Thinking: "synthetic thinking"}}},
				{Role: canonical.RoleUser, Content: []canonical.ContentBlock{{Type: canonical.ContentText, Text: "Follow up"}}},
			},
			MaxTokens: int64Ptr(1024),
		}
		...
		if strings.Contains(string(body), `"content":null`) {
			t.Fatalf("Request body must not contain null content: %s", string(body))
		}
		...
		if len(areq.Messages) != 2 {
			t.Fatalf("Expected only user messages after omitting empty assistant thinking, got %d messages", len(areq.Messages))
		}
	})
