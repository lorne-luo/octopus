...
func TestHandler(c *gin.Context) {
	...
	if canonicalReq.Stream {
		clientAdapter := adapter.GetClient(adapter.ClientOpenAIChat)
		if clientAdapter == nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to create client adapter"})
			return
		}
		handleTestStreamResponse(c, response, providerAdapter, clientAdapter)
	} else {
...
// handleTestStreamResponse converts provider SSE into OpenAI-compatible chunks.
func handleTestStreamResponse(c *gin.Context, response *http.Response, providerAdapter adapter.ProviderAdapter, clientAdapter adapter.ClientAdapter) {
	...
	for {
		...
		case r, ok := <-results:
			...
			chunk, err := providerAdapter.ParseStreamChunk(ctx, []byte(r.data))
			if err != nil {
				log.Warnf("[test] failed to parse stream chunk: %v", err)
				return
			}
			if chunk == nil || (!chunk.Done && len(chunk.Deltas) == 0) {
				continue
			}

			clientData, err := clientAdapter.FormatStreamChunk(ctx, chunk)
			if err != nil {
				log.Warnf("[test] failed to format stream chunk: %v", err)
				return
			}
			if len(clientData) == 0 {
				continue
			}
			if _, err := c.Writer.Write(clientData); err != nil {
				log.Warnf("[test] failed to write stream chunk: %v", err)
				return
			}
			c.Writer.Flush()
			if chunk.Done {
