...
- [ ] **`canonical/request.go`**: Define `RequestKind`, `Request`, `TransformHints`, `AudioConfig`, `OutputConfig`, `ReasoningConfig`, `StreamOptions`.
- [ ] **`canonical/response.go`**: Define `Response`, `Error`, `Chunk`, `Choice`, `ChoiceDelta`.
- [ ] **`canonical/message.go`**: Define `Role`, `ContentType`, `Message`, `ContentBlock` (with interleaved thinking support).
- [ ] **`canonical/tool.go`**: Define `Tool`, `ToolCall`, `ImageGenerationConfig`, `ToolChoice`.
- [ ] **`canonical/media.go`**: Define `MediaContent` (Image, Audio, File, Document).
- [ ] **`canonical/usage.go`**: Define `Usage`, `CacheCreationDetails`, `ServerToolUsage`, `PromptTokensDetails`, `CompletionTokensDetails`.

### 1.2 `transformer2/adapter/` Interfaces & Infrastructure
- [ ] **`adapter/adapter.go`**: Define `ClientAdapter` and `ProviderAdapter` interfaces.
- [ ] **`adapter/registry.go`**: 
...
- [ ] **`adapter/aggregator.go`**: Implement `StreamAggregator` (chunk concatenation, usage merging, last-wins for ReasoningSignature).
...
- [ ] **Wire Types**: Define `types_chat.go` (`ChatCompletionRequest`, `ChatCompletionResponse`, `ChatCompletionChunk`).
- [ ] **Provider Adapter (`provider_chat.go`)**: 
...
- [ ] **Quirks/Interceptors**: Implement `quirks/quirks.go` and specific overrides (e.g., `cerebras.go`, `groq.go`, Qwen `enable_thinking`).

### 2.2 OpenAI Responses API Adapter (`transformer2/adapter/openai/`)
_Note: Production critical (~900 lines of stateful streaming)._
- [ ] **Wire Types**: Define `types_responses.go` (`ResponsesRequest`, `ResponsesResponse`, `ResponsesStreamEvent` etc.).
...
- [ ] **Wire Types**: Define `types.go` (`MessageRequest`, `MessageParam`, `MessageContentBlock`, `StreamEvent`).
...
- [ ] **Provider Adapter (`provider.go`) & `convert.go`**: 
  - `BuildRequest`: 
...
- [ ] **Wire Types**: Move and clean up types into `adapter/gemini/types.go`.
- [ ] **Provider Adapter (`provider.go`) & `convert.go`**: 
...
- [ ] Implement `types_embedding.go`, `client_embedding.go`, and `provider_embedding.go`.
...
- [ ] **Call Site Updates (`internal/relay/relay.go`)**:
...
- [ ] **Metrics Refactor (`internal/relay/metrics.go`)**:
...
- [ ] **End-to-End Testing**: Verify SSE streams, Function Calling, Native Token Counting cross-provider, and Error format matching.
