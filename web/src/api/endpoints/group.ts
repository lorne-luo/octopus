 ...
 * 速度测试结果
 */
export interface GroupItemSpeed {
 ...
 *
 * triggerSpeedTest.mutate(1); // 触发 groupId=1 的速度测试
 */
export function useTriggerGroupSpeedTest() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      logger.log('速度测试触发成功:', data);
      // Invalidate and refetch group list to get updated speed data
      queryClient.invalidateQueries({ queryKey: ['groups', 'list'] });
    },
    onError: (error) => {
      logger.error('速度测试触发失败:', error);
    },
  });
}
