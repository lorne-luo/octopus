...
function ModelPickerSection({
  ...
  const channels = useMemo(() => {
    if (!Array.isArray(modelChannels)) {
      return [];
    }
    ...
    return Array.from(byId.values())
      .map((c) => ({
        ...c,
        models: [...c.models].sort((a, b) => a.name.localeCompare(b.name)),
      }))
...
export function GroupEditor({
  ...
  const [mode, setMode] = useState<GroupMode>(
    (initial?.mode ?? 1) as GroupMode,
  );
  const [firstTokenTimeOut, setFirstTokenTimeOut] = useState<number>(
    initial?.first_token_time_out ?? 0,
  );
  const [sessionKeepTime, setSessionKeepTime] = useState<number>(
    initial?.session_keep_time ?? 0,
  );
  ...
  const { matchedModelChannels, regexError } = useMemo(() => {
    ...
    if (regexKey) {
      try {
        ...
        return {
          matchedModelChannels: modelChannels.filter((mc) => re.test(mc.name)),
          regexError: "",
        };
      } catch (e) {
        return {
          matchedModelChannels: [],
          regexError: (e as Error)?.message ?? "Invalid regex",
        };
      }
    }
    if (!groupKey) return { matchedModelChannels: [], regexError: "" };
    return {
      matchedModelChannels: modelChannels.filter((mc) =>
        matchesGroupName(mc.name, groupKey),
      ),
      regexError: "",
    };
  ...
  const isValid =
    groupKey.length > 0 && selectedMembers.length > 0 && !regexError;
