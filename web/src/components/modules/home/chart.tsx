...
export function StatsChart() {
    ...
    const totals = useMemo(() => {
        if (period === '1') {
            if (!statsHourly) return { requests: 0, tokens: 0, cost: 0 };
            return {
    ...
    const chartConfig = {
        total_token: { label: t('totalTokens') },
    };
        ...
        <div className="rounded-3xl bg-card border-card-border border pt-2 pb-0 text-card-foreground custom-shadow">
            ...
            <ChartContainer config={chartConfig} className="h-40 w-full" >
                <AreaChart accessibilityLayer data={chartData}>
                    ...
                    {/* <ChartTooltip cursor={false} content={<ChartTooltipContent indicator="line" fo  rmatter={(value) => 'Tokens '+formatCount(value as number).formatted.value + formatCount(value as number).formatted.unit} />} /> */}
                    <Area type="monotone" dataKey="total_token" stroke="var(--chart-1)" fill="url(#fillTotalToken)" />
                </AreaChart>
            </ChartContainer>
        </div>
    );
